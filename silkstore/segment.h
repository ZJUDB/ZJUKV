...
class SegmentBuilder {
 public:
  ...
  // REQUIRES: all mini runs has finished building through pairs of
  // StartMiniRun() and FinishMiniRun().
  Status Finish();
...
class Segment {
 public:
  ...
  // boolean value indicating whether the run number has been invalidated
  // through InvalidateMiniRun() previously. This function is mainly used in
  // garbage collection.
  void ForEachRun(std::function<bool(int run_no, MiniRunHandle run_handle,
...
class SegmentManager {
 public:
  ...
  // This function waits for old readers to the segment to exit before
