...
class DBTest {
 ...
 public:
  ...
  int NumTableFilesAtLevel(int level) { return 0; }
...
TEST(DBTest, RandomKeyTest) {
  ...
  for (int i = 0; i < kNumOps; i++) {
    ...
    for (int j = 0; j < 1; ++j) {
      ...
      if (res != ans) {
        fprintf(stderr,
                "Key %s has wrong value %s, correct value %s, last inserted "
                "key %s %d\n",
                keys[idx].c_str(), res.c_str(), ans.c_str(),
                keys[i % kNumKVs].c_str(), i);
        // res = Get(keys[idx]);
      }
      ASSERT_EQ(res, ans);
    }
  ...
  for (int i = 0; i < kNumOps; ++i) {
    ...
    if (res != ans) {
      fprintf(stderr,
              "Key %s has wrong value %s, correct value %s, last inserted key "
              "%s %d\n",
              keys[idx].c_str(), res.c_str(), ans.c_str(),
              keys[i % kNumKVs].c_str(), i);
      // res = Get(keys[i]);
...
//        ASSERT_OK(Put("foo", "v1"));
//        std::string val;
//        ASSERT_TRUE(db_->GetProperty("leveldb.approximate-memory-usage",
//        &val)); int mem_usage = atoi(val.c_str()); ASSERT_GT(mem_usage, 0);
...
//        for (int i = 0; i < 2; i++) {
//            std::string key = (i == 0) ? std::string("foo") : std::string(200,
//            'x'); ASSERT_OK(Put(key, "v1")); const Snapshot* s1 =
//            db_->GetSnapshot(); ASSERT_OK(Put(key, "v2")); ASSERT_EQ("v2",
//            Get(key)); ASSERT_EQ("v1", Get(key, s1));
//            dbfull()->TEST_CompactMemTable();
...
//            std::string key = (i == 0) ? std::string("foo") : std::string(200,
//            'x'); ASSERT_OK(Put(key, "v1")); const Snapshot* s1 =
//            db_->GetSnapshot(); const Snapshot* s2 = db_->GetSnapshot(); const
//            Snapshot* s3 = db_->GetSnapshot(); ASSERT_OK(Put(key, "v2"));
//            ASSERT_EQ("v2", Get(key));
//            ASSERT_EQ("v1", Get(key, s1));
...
//            ASSERT_LE(compaction_count, 100) << "could not fill levels 0 and
//            2"; compaction_count++; Put("a", "begin"); Put("z", "end");
//            dbfull()->TEST_CompactMemTable();
//        }
//
...
//        it ASSERT_OK(Put("foo", "v1"));                         // Goes to 1st
//        log file ASSERT_OK(Put("big1", std::string(10000000, 'x')));  // Fills
//        memtable ASSERT_OK(Put("big2", std::string(1000, 'y')));      //
//        Triggers compaction ASSERT_OK(Put("bar", "v2")); // Goes to new log
//        file
//
//        Reopen(&options);
//        ASSERT_EQ("v1", Get("foo"));
...
//            Reopen(&options);
//
//            for (int compact_start = 0; compact_start < N; compact_start +=
//            10) {
//                for (int i = 0; i < N; i += 10) {
//                    ASSERT_TRUE(Between(Size("", Key(i)), S1*i, S2*i));
//                    ASSERT_TRUE(Between(Size("", Key(i)+".suffix"), S1*(i+1),
//                    S2*(i+1))); ASSERT_TRUE(Between(Size(Key(i), Key(i+10)),
//                    S1*10, S2*10));
//                }
//                ASSERT_TRUE(Between(Size("", Key(50)), S1*50, S2*50));
//                ASSERT_TRUE(Between(Size("", Key(50)+".suffix"), S1*50,
//                S2*50));
//
...
//            so. ASSERT_OK(dbfull()->TEST_CompactMemTable());
//        }
//
//        // Check sizes across recovery by reopening a few times
//        for (int run = 0; run < 3; run++) {
...
//    ASSERT_EQ(NumTableFilesAtLevel(last), 1);   // foo => v1 is now in last
//    level
//
//    // Place a table at level last-1 to prevent merging with preceding
//    mutation Put("a", "begin"); Put("z", "end");
//    dbfull()->TEST_CompactMemTable();
...
//    do {
//        ASSERT_EQ(config::kMaxMemCompactLevel, 2) << "Fix test to match
//        config";
//
//        // Fill levels 1 and 2 to disable the pushing of new memtables to
//        levels > 0. ASSERT_OK(Put("100", "v100")); ASSERT_OK(Put("999",
//        "v999")); dbfull()->TEST_CompactMemTable(); ASSERT_OK(Delete("100"));
...
{ ToNumber(*s);     // Check format ToNumber(l);      // Check format
    ...
    private:
        static int ToNumber(const Slice& x) {
            ...
            int val;
            char ignored;
            ASSERT_TRUE(sscanf(x.ToString().c_str(), "[%i]%c", &val, &ignored)
== 1)
                    << EscapeString(x);
            return val;
        }
...
iter, so
                        // we have multiple entries in the write batch for the
same key
                    }
                    if (rnd.OneIn(2)) {
...
nullptr)); ASSERT_TRUE(CompareIterators(step, &model, db_, model_snap,
db_snap));
