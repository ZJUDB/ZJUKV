...
TEST(MinirunTest, BuilderTestLarge) {
  ...
  for (int i = 0; i < kNum; ++i) {
    ...
    keys[i] = string(key_buf, 9);
    values[i] = RandomString(&rnd, 100);
...
TEST(MinirunTest, MiniRunIndexEntryTest) {
  ...
  string buf1;
  auto e1 = MiniRunIndexEntry::Build(0, 0, Slice(index_block_contents),
                                     Slice(filter_block_contents), 0, &buf1);
  ...
  ASSERT_EQ(e1.
            ...
            ToString(),
            filter_block_contents

  );

  string buf2;
  auto e2 = MiniRunIndexEntry::Build(2, 1, Slice(index_block_contents),
                                     Slice(filter_block_contents), 0, &buf2);
  ASSERT_EQ(e2.
...
TEST(MinirunTest, LeafIndexEntryTest) {
...

  {
    ...
    for (int i = 0; i < eNum; ++i) {
      auto e1 = MiniRunIndexEntry::Build(i, i, Slice(std::to_string(i)),
                                         Slice(std::to_string(i)), 0, &bufs[i]);
      LeafIndexEntryBuilder::AppendMiniRunIndexEntry(
          base_entry, e1, &new_entry_bufs[i], &new_entry);
    ...
    );
    {
      for (int i = 0; i < eNum; ++i) {
        ...
        auto e = MiniRunIndexEntry::Build(
            eNum * 2, eNum * 2, Slice(std::to_string(eNum * 2)),
            Slice(std::to_string(eNum * 2)), 0, &bufs[0]);
        int replace_start = i;
        ...
        Status s = LeafIndexEntryBuilder::ReplaceMiniRunRange(
            base_entry, replace_start, replace_end, e, &new_entry_bufs[0],
            &new_entry);
        ASSERT_TRUE(s.
        ...
        auto entries = new_entry.GetAllMiniRunIndexEntry(
            LeafIndexEntry::TraversalOrder::forward);
        ASSERT_EQ(entries.
...

    {
      for (int i = 0; i < eNum; ++i) {
        ...
        Status s = LeafIndexEntryBuilder::RemoveMiniRunRange(
            base_entry, remove_start, remove_end, &new_entry_bufs[0],
            &new_entry);
        ASSERT_TRUE(s.
