...
class MiniRun {
 public:
  ...
  MiniRun(const Options* options, RandomAccessFile* file, uint64_t off,
          uint64_t size, Block& index_block);

  static Iterator* BlockReader(void*, const ReadOptions&, const Slice&);

  // Returns a iterator ranging over a specific block in the run
  Iterator* NewIteratorForOneBlock(const leveldb::ReadOptions&,
                                   BlockHandle handle);
...
class MiniRunBuilder {
 public:
  ...
  // building in *file starting at file_offset.  Does not close the file.  It is
  // up to the caller to close the file after calling Finish().
  MiniRunBuilder(const Options& options, WritableFile* file,
