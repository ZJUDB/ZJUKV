...
SilkStore::SilkStore(const Options &raw_options, const std::string &dbname)
    : env_(raw_options.env), internal_comparator_(raw_options.comparator),
      ...
      dbname_(dbname), leaf_index_(nullptr), db_lock_(nullptr),
      shutting_down_(nullptr), background_work_finished_signal_(&mutex_),
      mem_(nullptr), imm_(nullptr), logfile_(nullptr), logfile_number_(0),
      log_(nullptr), max_sequence_(0),
      memtable_capacity_(options_.write_buffer_size), seed_(0),
      tmp_batch_(new WriteBatch), background_compaction_scheduled_(false),
...
Iterator *SilkStore::NewIterator(const ReadOptions &ropts) {
  ...
  SequenceNumber seqno =
      ropts.snapshot ? dynamic_cast<const SnapshotImpl *>(ropts.snapshot)
                           ->sequence_number()
                     : max_sequence_;
...
public:
  GroupedSegmentAppender(int num_groups, SegmentManager *segment_manager,
      ...
      : builders(num_groups), segment_manager(segment_manager),
        options(options), gc_on_segment_shortage(gc_on_segment_shortage) {}
  ...
  seg->ForEachRun([&, this](int run_no, MiniRunHandle run_handle,
    ...
    if (block_it->Valid()) {
      ...
      leaf_index_entry.ForEachMiniRunIndexEntry(
          [&run_idx_in_index_entry, run_no,
           seg_id](const MiniRunIndexEntry &minirun_index_entry, uint32_t idx) {
...
void SilkStore::ProcessOneLeaf(
  ...
  auto SplitLeaf = [&grouped_segment_appender, &leaf_index_wb,
                    this](const LeafIndexEntry &leaf_index_entry,
