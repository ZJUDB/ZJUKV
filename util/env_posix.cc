...
public:
  ...
  void ScheduleDelayedTask(std::function<void()> f,
                           int delay_in_micros) override;
...
private:
  ...
  std::queue<std::pair<std::function<void()>, int>>
      delayed_work_queue_ GUARDED_BY(background_work_mutex_);
...
PosixEnv::PosixEnv()
    : background_work_cv_(&background_work_mutex_),
      ...
      started_background_delay_work_thread_(false), mmap_limiter_(MaxMmaps()),
...
void PosixEnv::ScheduleDelayedTask(std::function<void()> f,
                                   int delay_in_micros) {
  background_work_mutex_.Lock();
  ...
  if (!started_background_delay_work_thread_) {
    ...
    std::thread background_delay_work_thread(
        PosixEnv::BackgroundDelayWorkThreadEntryPoint, this);
...
void PosixEnv::BackgroundDelayWorkThreadMain() {
  while (true) {
    ...
    auto background_work_function = delayed_work_queue_.front().first;
...
Env *Env::NewPosixEnv() { return new PosixEnv; }
} // namespace leveldb
