...
template <typename Key> class bloom1 {
public:
  using key_type = Key;
...
public:
  ...
  void Add(const Slice key) { b.insert(key, BloomHashMurmur); }

  // Check if the key is possibly in the filter
  bool KeyMayMatch(const Slice &key) { return b.check(key, BloomHashMurmur); }
